...
export function formatSetupPrompt(state: SidekickSetupState): string {
  return [
    ...
    '5. Each key in `agents` becomes an MCP tool named `ask_<key>` and a sidekick agent name. Use memorable aliases such as gemini, claude, codex, deepseek, or kimi.',
    ...
    '10. Sidekick maps effort to Claude `--effort` (`low`, `medium`, `high`), Codex `--config model_reasoning_effort=...` (`minimal`, `low`, `medium`, `high`), and OpenCode `--variant` (simple variant name). Gemini CLI has no direct reasoning-effort flag, so effort overrides are rejected for Gemini agents.',
